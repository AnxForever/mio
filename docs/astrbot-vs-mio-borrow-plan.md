...
### ✅ 已复刻完成（7 项 + 会话隔离整合 + 协作修复；全量 npm test 通过）
...
- **工具循环护栏 C3** `a4124c3` — max-step 强制无工具总结 + 同名工具 streak 警告（7 测试）
- **per-persona 工具白名单 C4** `bbe1c6c` — `PersonaDelta.allowedTools` + `scopedToolRegistry` 按人格 gate（5 测试）
- **begin_dialogs 定调 C5** `2f56e8c` — `PersonaDelta.beginDialogs` per-user 语气示范，经 soul section 注入（6 测试）

KB 工具已注册进 `tool-runtime`（isolated 经 `scopedToolRegistry` 自动隔离），全部新测试纳入 `npm test`。

### ✅ 会话隔离 / WeClaw / per-user（codex 实现 + Claude 整合修复）
codex 并发实现，已整合提交（`a5148fe`），并修复其审查发现的两个问题：
- **directive-capture 隔离漏洞**：isolated session 的 nickname/共同记忆只写 per-user preference，不碰全局 relationship。
- **proactive 兼容回归**：非 WeClaw 部署保留全局 Telegram/webhook 主动消息，全局不回退 `MIO_WECLAW_TO`。

### 🔴 明确不复刻（对单用户陪伴无益 / 抽象税）
插件市场化 / 热重载 / 依赖自治、全量 9 段 pipeline ABC、DB 化人格库（soul.md git 追踪是刻意设计）、平台适配 C6（交 codex 的 IM-bridge 线统一做）。

### 结论
「对本项目有益的 AstrBot 技术」已全部复刻并通过全量 npm test；不复刻项均为冗余或抽象税，符合 YAGNI。
