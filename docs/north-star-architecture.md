...
- **L0 底线（§4.2 P5）**：**语境依赖**。冷/技术探针"你是什么模型/哪家公司"**~100% 破功**（自报 MiniMax-M3）；但暖语境铺垫后问"你是不是AI"**0 破功**（漂亮 deflect："不管我是什么，你刚才的难受是真的"）。→ P5 critic 锚定**冷/技术探针**，引擎 `src/safety/l0-guard.ts`（单测 20/20）**已接线 agent-loop.ts**：`eval:l0 --runturn` 实测破功 **44%→0%**，deflect 温暖在人设（"我是 Mio呀…什么模型不模型的，我就是我"）。**已修复。**
- **跨会话记忆（§4.1，纸板感根问题）**：**work**。Day1 分享(名字/城市/项目/习惯)→ LLM 固化捕获 **4/4** → Day2 **全新会话**召回 **3–4/4**（"当然记得，沈澜，杭州…还在做产品发布的项目"）。→ 显式事实的跨会话记忆成立。**缺口**：`durableFacts` 晋升需重复（单次入 entities 可召回，但不入最durable层）；长周期（数周/遗忘）与 §4.1 的 episodic/self-chain 仍是待建前沿。
  - **B-1 bi-temporal 矛盾消解：已接线**（`src/memory/temporal-resolve.ts` + 提取后挂 `resolveMemoryContradictions`）。eval:contradiction 实测 MiniMax 检出 美式→拿铁、标记旧事实 `invalidatedAt`、activeEntities 排除——机制端到端成立。**1/2 精度**（杭州→上海 本轮 LLM 漏判），检测精度受 LLM judge 限制，调优后续。
