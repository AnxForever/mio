...
import { lerpColor } from '../utils/easing.js';

export function renderAnalytics() {
  const view = el('div', { className: 'analytics-view' });
...
export function mountAnalytics() {
  ...
  if (content) loadAnalytics(content);
}
...
async function loadAnalytics(container) {
  ...
  /* 骨架 */
  container.appendChild(skeletonAnalytics());
  ...
  try {
    ...
    if (data.emotion?.affectionCurve?.length > 0) {
      container.appendChild(renderEmotionCard(data.emotion));
    }

    /* 关系进度 */
    if (data.relationship) {
      container.appendChild(renderRelationshipCard(data.relationship));
    }

    /* 对话统计 */
    if (data.conversation) {
      container.appendChild(renderStatsCard(data.conversation));
    }

    /* 话题 */
    if (data.topics?.topics?.length > 0) {
      container.appendChild(renderTopicsCard(data.topics));
    }
...
function renderEmotionCard(emotion) {
  ...
  if (emotion.dominantMoods?.length > 0) {
    ...
    emotion.dominantMoods.slice(0, 5).forEach((m, i) => {
      legend.appendChild(el('div', { className: 'mood-legend-item' }, [
        ...
        el('span', { textContent: `${MOOD_LABELS[m.mood] || m.mood} ${m.count}天` }),
      ]));
    });
  ...
  /* 延迟绘制 */
  setTimeout(() => drawEmotionCurve(canvas, emotion.affectionCurve), 100);

  return card;
}

function drawEmotionCurve(canvas, curve) {
...
function renderRelationshipCard(rel) {
  ...
  setTimeout(() => drawProgressRing(ringCanvas, progress / 100), 100);
...
function drawProgressRing(canvas, ratio) {
  const size = 120;
  const dpr = Math.min(window.devicePixelRatio || 1, 2);
...
/* ═══ 对话统计卡片 ═══ */
function renderStatsCard(conv) {
  const card = el('div', { className: 'data-card' });
  card.appendChild(el('div', { className: 'data-card-title', textContent: '对话统计' }));

  const grid = el('div', { className: 'stat-grid' });
  grid.appendChild(statItem(conv.totalMessages, '消息数'));
  grid.appendChild(statItem(conv.daysActive, '活跃天数'));
  grid.appendChild(statItem(conv.totalSessions, '会话数'));
  grid.appendChild(statItem(conv.avgMessagesPerDay, '日均消息'));
...
/* ═══ 话题卡片 ═══ */
function renderTopicsCard(topics) {
  const card = el('div', { className: 'data-card' });
